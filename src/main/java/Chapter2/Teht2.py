...
class ReservedMemory():
    ...
    def copy(self, mem_source:ReservedMemory, count:int=None, source_index:int=0, destination_index:int=0) -> None:
        ...
        - source_index: Source's start index where to copy the content.
                        Optional. Default: 0
        - destination_index: Destination's start index to copy the content.
                             Optional. Default: 0
...
class IntArray():
    ...
    def remove(self, index:int) -> int:
        ...
        # Reserve a new memory area with the new size (if necessary)
        if self._size:
            # It is _bytes_per_element smaller than the current one
            new_resmem = ReservedMemory(self._size*self._bytes_per_element)
            ...
            if self._resmem:
                ...
                if index < self._size:
                    new_resmem.copy(self._resmem, source_index=(index+1)*self._bytes_per_element, destination_index=index*self._bytes_per_element)
        else:
            new_resmem = None
